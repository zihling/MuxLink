...
if (!defined $lik0 || !defined $lik1) {
    print "Warning: Missing likelihood for key=$key output=$output path0=$path0 path1=$path1\n";
} else {
    print "Key=$key, Output=$output, Path0=$path0, Likelihood0=$lik0, Path1=$path1, Likelihood1=$lik1\n";
}
